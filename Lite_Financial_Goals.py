...
def generate_blank_templates():
    ...
    with pd.ExcelWriter(output) as writer:
        pd.DataFrame(columns=["Source", "Monthly", "Growth %/yr", "Start Year", "End Year"]).to_excel(writer, sheet_name="Income", index=False)
        pd.DataFrame(columns=["Name", "Monthly", "Inflation %", "Start Year", "End Year"]).to_excel(writer, sheet_name="Expenses", index=False)
        pd.DataFrame(columns=["Goal Name", "Cost Today", "Inflation %", "Start Year", "End Year", "Frequency (yrs)"]).to_excel(writer, sheet_name="Goals", index=False)
        pd.DataFrame(columns=["Asset Name", "Asset Type", "Class", "Purch Date", "Invested", "Cur Val", "Mat Amt", "Mat Date", "Gross CAGR %", "Tag Goal", "SWP /mo", "SWP Yr"]).to_excel(writer, sheet_name="Assets", index=False)
        pd.DataFrame(columns=["Loan Name", "Outstanding Principal", "Interest Rate %", "Remaining Months"]).to_excel(writer, sheet_name="Liabilities", index=False)
...
def generate_all_tables_excel_bytes():
    ...
    try:
        with pd.ExcelWriter(output) as writer:
            ...
            if inc_df.empty: inc_df = pd.DataFrame(columns=["Source", "Monthly", "Growth %/yr", "Start Year", "End Year"])
            ...
            if exp_df.empty: exp_df = pd.DataFrame(columns=["Name", "Monthly", "Inflation %", "Start Year", "End Year"])
            ...
            if goal_df.empty: goal_df = pd.DataFrame(columns=["Goal Name", "Cost Today", "Inflation %", "Start Year", "End Year", "Frequency (yrs)"])
            ...
            if liab_df.empty: liab_df = pd.DataFrame(columns=["Loan Name", "Outstanding Principal", "Interest Rate %", "Remaining Months"])
            ...
            if sum_df.empty: sum_df = pd.DataFrame(columns=["Goal", "Start", "End", "Cumulative Cost", "Target Cost (Used)", "Net Present Value", "Allocated (Current Corpus)", "Allocated (Future Sweep)", "% Met", "Status", "Current Add'l Contribution Required"])
            ...
            if granular_rows:
                ...
                for r in granular_rows:
                    g = r.get("Goal", "")
                    if g == "TOTAL": continue
                    if g not in grouped_data: grouped_data[g] = []
            ...
            if df_hierarchical.empty: df_hierarchical = pd.DataFrame(columns=["Row Labels", "Asset Type", "Asset Class", "Allocated Amount", "% of Goal's current Funding"])
...
def asset_swp_start_rel(a):
    ...
    if raw <= 0: return 0
    if raw > 1000: return cal_to_rel(raw)
    return raw
...
def get_dashboard_eval_year():
    ...
    if not active_in_base:
        ...
        if future_years: return min(future_years)
    return base_yr

def total_monthly_income():
    eval_yr = get_dashboard_eval_year()
    return sum(e["monthly"] for e in st.session_state.income if int(e.get("start_year", eval_yr) or eval_yr) <= eval_yr <= int(e.get("end_year", 2100) or 2100))

def total_monthly_expense():
    eval_yr = get_dashboard_eval_year()
    return sum(e["monthly"] for e in st.session_state.expenses if int(e.get("start_year", eval_yr) or eval_yr) <= eval_yr <= int(e.get("end_year", 2100) or 2100))

def avg_inflation():
    eval_yr = get_dashboard_eval_year()
    active = [e for e in st.session_state.expenses if int(e.get("start_year", eval_yr) or eval_yr) <= eval_yr <= int(e.get("end_year", 2100) or 2100)]
...
def liabilities_at_year(y): return sum(liability_value_at_year(l, y) for l in st.session_state.liabilities)
...
def goal_names(): return [g["name"] or f"Goal {i+1}" for i, g in enumerate(st.session_state.goals)]
...
def goal_frequency(g): return max(int(g.get("frequency", 0) or 0), 0)

def goal_uses_cumulative(g): return goal_frequency(g) > 0
...
def goal_npv(g, wcagr_pct):
    ...
    if wcagr_pct <= 0: return sum(cost for _, cost in occs)
    r = wcagr_pct / 100
...
Upload your financial goals via Excel. Define the cost in today's money, a start year, end year, and recurrence frequency.
One-time goals (a home purchase) use the same start and end year with frequency 0. Recurring goals
(school fees, retirement) set frequency to 1 for annual, 2 for every two years, etc.

<br/><br/><b style="color:#93c5fd;">Step 3 — Assets</b><br/>
Upload your investments and savings. Set custom tax rates for each asset class in the Settings tab and toggle <b>Apply Automatic Tax Drag</b> to dynamically convert your gross CAGRs into net, post-tax returns across the app. Tag an asset to one or more goals to earmark
it; untagged assets form a shared pool that fills any remaining gaps.
...
with tab_settings:
    ...
    # i) Download Blank Master Template (NEW)
    c_set_0, c_desc_0 = st.columns([1, 1])
    with c_set_0:
        st.markdown("#### i) Download Blank Input Template")
        st.markdown('<div style="color: #059669; font-weight: bold; margin-bottom: 10px;">Excel-First Workflow: All data must be imported via Excel.</div>', unsafe_allow_html=True)
        
        template_bytes = generate_blank_templates()
        if st.download_button("📥 Download Blank Master Template (Excel)", data=template_bytes, file_name="Financial_Planner_Master_Template.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True, key="download_blank_template"):
            st.toast("✅ Blank template downloaded successfully!")
    ...
    with c_set_1:
        ...
        with btn_col1:
            if st.download_button("⬇️ Download JSON", data=json.dumps(export_data, indent=2), file_name="financial_planner_data.json", mime="application/json", use_container_width=True, key="download_json_settings"):
    ...
    with c_desc_1:
        st.markdown("#### Instructions")
        st.info("Because this planner runs entirely in your browser for privacy, your data is lost when you close the tab. Download your session as a JSON file to keep your configuration safe, or use it to migrate your data between sessions.")
    ...
    with c_desc_4:
        st.markdown("#### Instructions")
        st.info("Download all your entered inputs (Income, Expenses, Goals, Assets, Liabilities) and the calculated summary tables into a single multi-sheet Excel file. This includes your fully generated Cash Flow ledger.")
                ...
                if suspect_goals:
                    for g in suspect_goals:
                        ...
                        st.error(
                            ...
                            f"recur (e.g. annual retirement expenses, yearly fees), update the Frequency "
                            f"in your Excel sheet and re-upload. If it's genuinely a one-time cost, set End Year "
                            f"equal to Start Year."
                        )
                if zero_cagr_val > 0:
                    st.warning(
                        ...
                        f"weighted CAGR to {weighted_cagr():.1f}%. Please update the expected returns in your Excel sheet for: " +
    ...
    if st.session_state.goals:
        ...
        if suspect:
            ...
            st.error(
                ...
                f"Update Frequency ≥ 1 in your Excel if this should recur."
            )
...
with tab_assets:
    ...
    if st.session_state.get("_cagr_defaults_applied"):
        ...
        st.warning(
            ...
            f"applied. Please review your Excel upload if this was unintended:** {names_str}"
        )
...
with tab_liab:
    ...
    # Render as read-only view
    st.dataframe(liab_df, hide_index=True, use_container_width=True)
